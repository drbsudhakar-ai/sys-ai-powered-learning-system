...
import { commitExternalKnowledgeImport, createCourseTeachingPackRevision, decideCourseTeachingPack, getApiErrorMessage, getCourseKnowledgeStudio, getExternalKnowledgeTemplate, getPilotKnowledgeApprovalPreview, pilotApproveKnowledgePackages, previewExternalKnowledgeImport, saveSubjectDeliveryGuide } from "../../src/api";
...
export default function CourseKnowledgeStudio({ courseId, canActivate, backHref }) {
  ...
  const [pilotPreview, setPilotPreview] = useState(null); const [pilotCourseCode, setPilotCourseCode] = useState("");
  ...
  async function previewPilotApproval() {
    setBusy("pilot-preview"); setError(""); setSuccess("");
    try { const { data } = await getPilotKnowledgeApprovalPreview(courseId); setPilotPreview(data); }
    catch (requestError) { setError(getApiErrorMessage(requestError, "Unable to preview controlled-pilot eligibility.")); }
    finally { setBusy(""); }
  }
  async function approvePilotPackages() {
    if (!pilotPreview?.eligible_count) return;
    setBusy("pilot-approve"); setError(""); setSuccess("");
    try {
      const { data } = await pilotApproveKnowledgePackages(courseId, { course_code: pilotCourseCode.trim(), reason: pilotReason.trim(), expected_package_ids: pilotPreview.expected_package_ids });
      await load(); await previewPilotApproval();
      setSuccess(`${data.approved_count} knowledge package(s) approved for controlled pilot use. Independent expert verification remains pending.`);
    } catch (requestError) { setError(getApiErrorMessage(requestError, "Unable to approve the eligible packages for controlled pilot use.")); }
    finally { setBusy(""); }
  }
  ...
  {canActivate && <section className={styles.card}><h2>5. Controlled Pilot Knowledge Approval</h2><p>Approve academically complete draft packages for controlled demonstration. This does not represent independent expert or institutional approval.</p><div className={styles.actions}><button className={styles.secondary} onClick={previewPilotApproval} disabled={Boolean(busy)}>{busy === "pilot-preview" ? "Checking eligibility…" : "Preview pilot eligibility"}</button></div>{pilotPreview && <><div className={pilotPreview.eligible_count > 0 ? styles.notice : styles.success}><strong>{pilotPreview.eligible_count} of {pilotPreview.package_count} package(s) eligible</strong><p>{pilotPreview.declaration}</p></div>{pilotPreview.items?.some((item) => item.errors?.length) && <ul className={styles.validation}>{pilotPreview.items.filter((item) => item.errors?.length).map((item) => <li key={item.package_id}><strong>{item.subject_name} — {item.topic_name}:</strong> {item.errors.join("; ")}</li>)}</ul>}{pilotPreview.eligible_count > 0 && <div className={styles.grid}><Field label={`Exact course code (${pilotPreview.course_code})`} area={false} value={pilotCourseCode} onChange={setPilotCourseCode} /><Field label="Pilot approval reason" value={pilotReason} onChange={setPilotReason} wide /></div>}<div className={styles.actions}><button className={styles.primary} onClick={approvePilotPackages} disabled={Boolean(busy) || !pilotPreview.eligible_count || pilotCourseCode.trim() !== pilotPreview.course_code || pilotReason.trim().length < 20}>{busy === "pilot-approve" ? "Approving…" : "Approve eligible packages for pilot"}</button></div></>}</section>}
