...
test("Knowledge Studio exposes policy, guides, coverage, and honest future capabilities", () => {
  ...
  for (const text of ["Course Delivery Policy", "Subject Delivery Guides", "External Knowledge Package Import", "Download course template", "Preview and validate", "Commit governed import", "Controlled Pilot Knowledge Approval", "Preview pilot eligibility", "Approve eligible packages for pilot", "P036.2C", "disabled"]) assert.match(ui, new RegExp(text));
...
test("course profile and coordinator responsibility cards link to Knowledge Studio", () => {
  ...
  for (const method of ["getCourseKnowledgeStudio", "createCourseTeachingPackRevision", "decideCourseTeachingPack", "saveSubjectDeliveryGuide", "getExternalKnowledgeTemplate", "previewExternalKnowledgeImport", "commitExternalKnowledgeImport", "getPilotKnowledgeApprovalPreview", "pilotApproveKnowledgePackages"]) assert.match(api, new RegExp(method));
});
